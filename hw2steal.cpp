...
void finish_task(Task*,int);
vector<list<Task*>> deques;
vector<int> rand_seed;
		...
		for (int i = 0; i < omp_get_num_threads(); i++){
			  ...
			  #pragma omp critical
			  {
				  if (!first_task){
					...
					rand_seed = vector<int>(num_threads,0);
			  ...
			  while(flag){
				...
				if (current_task){ //if task is ready
				  ...
				  if (current_task->task[3] == m){ //if task is small enough, do it immediately (base case)
					...
					finish_task(current_task, thread_ID);
					...
					#pragma omp critical
					{
					  deques[thread_ID].push_front(task4);
					  deques[thread_ID].push_front(task3);
					  deques[thread_ID].push_front(task2);
...
void finish_task(Task* current_task, int thread_ID){
