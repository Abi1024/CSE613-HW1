...
int main(){
  ...
  int r = 4;
	...
	cout << "\t Using implementation ikj (parallelize loops ij) " << endl;
    time = matrix_multiply(r,22);
    cout << "\t Time(ms): " << time*1000 << endl;
    out << time*1000 << ",";;
	if (time > max_time)
		max_time = time;

    cout << "\t Using implementation ikj (parallelize loops j) " << endl;
    time = matrix_multiply(r,23);
    cout << "\t Time(ms): " << time*1000 << endl;
    out << time*1000 << ",";;
    if (time > max_time)
      max_time = time;
	...
	cout << "\t Using implementation kij (parallelize loops ij) " << endl;
    time = matrix_multiply(r,52);
    cout << "\t Time(ms): " << time*1000 << endl;
    out << time*1000 << ",";;
    if (time > max_time)
      max_time = time;
  
	cout << "\t Using implementation kij (parallelize loops j) " << endl;
    time = matrix_multiply(r,53);
    cout << "\t Time(ms): " << time*1000 << endl;
    out << time*1000 << ",";
    if (time > max_time)
      max_time = time;

	if ((max_time > 60000)||(r == 9)){
		flag = false;
	}
		out << endl;
		r += 1;
