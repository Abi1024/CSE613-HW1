...
int main(){
	  ...
	  #pragma omp parallel
	  {
		...
		for (int i = 0; i < omp_get_num_threads(); i++){
			  ...
			  while(flag){
				...
				//printf("Checking if deque is empty ID:%d \n", thread_ID);
				#pragma omp critical
				{
				  if (deque.size() > 0){
					//printf("Deque is not empty ID:%d\n", thread_ID);
					current_task = deque.front();
					deque.pop_front();
					//printf("deque size:%d ID:%d\n",   deque.size(), thread_ID);
				  }else{
					//printf("Deque is empty ID:%d\n", thread_ID);
				  }
				}

				//printf("Checking if there is a task\n");
				if (current_task){ //if task is ready
				  //printf("Executing task: Z:%d X:%d Y:%d size:%d ID:%d\n", current_task->task[0],current_task->task[1],current_task->task[2],current_task->task[3],thread_ID);
				  if (current_task->task[3] == m){ //if task is small enough, do it immediately (base case)
					...
					//printf("Allocating tasks ID: %d\n", thread_ID);
					Task* task1 = new Task(vector<int>{Z,X,Y,size/2},current_task,0);
					//printf("Allocating tasks1 Trhhread ID: %d\n", thread_ID);
					...
					#pragma omp critical
					{
					  deque.push_front(task4);
					  deque.push_front(task3);
					  deque.push_front(task2);
					  deque.push_front(task1);
					}
					//printf("Tasks allocated\n");
				  }
				}
  ...
  for (int i = 0; i < global_n; i++){
	...
	double time = omp_get_wtime() - start_time;
	  ...
	  out << time*1000 << ",";
	  out << 2*pow(pow(2,r),3)/(time*pow(10,9));
	  out << endl;
	  if (time > 300){
		break;
	  }
